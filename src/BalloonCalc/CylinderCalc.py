...
#        print("------------------------------")
    ...
    # Durchmesser und Höhe berechnen:
    diameter = 2*(v_bal/(2*pi))**(1/3)
    height = diameter                   # Bei idealem Oberflächen/Volumen Verhältnis entspricht die Zylinderhöhe dem Durchmesser

    # Oberfläche berechnen:
    surface = 2*pi*(diameter/2)**2 + 4*pi*(diameter/2)**2
    return [v_bal, diameter, height, surface]
...
def calculate_payload(conditions_at_max_altitude,rho_lifting_gas_at_max_altitude,d,rho_hull,v_bal):
    ...
    mass_balloon_hull = 6 * pi * (v_bal / (2 * pi)) ** (2 / 3) * d * rho_hull
